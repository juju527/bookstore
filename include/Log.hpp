...
public:
  ...
  Finance operator-(Finance tmp) const {
    return (Finance){income - tmp.income, expend - tmp.expend};
  }
  friend std::ostream &operator<<(std::ostream &out, const Finance &f) {
    out << "+ " << fixed << setprecision(2) << f.income << " - " << fixed
        << setprecision(2) << f.expend;
    return out;
  }
};
class FinanceInfo {
public:
  ...
  Finance finance;
  FinanceInfo() { len = 0; }
  FinanceInfo(String30 userid, int l, array<char, operlen> oper, Finance fin) {
    UserID = userid, len = l, Operation = oper, finance = fin;
  }
...
public:
  ...
  array<char, operlen> Operation;
  LogInfo() { len = 0; }
  LogInfo(String30 userid, int l, array<char, operlen> oper) {
    UserID = userid, len = l, Operation = oper;
  }
...
public:
  ...
  void addFinance(double inc, double exp, String30 userid, int len,
    ...
    Finance lst;
    if (num)
      financeStorage.readorder(lst, num);
  ...
  bool showFinance(int Count) {
    ...
    if (num - Count)
      financeStorage.readorder(L, num - Count);
    cout << (R - L) << endl;
    financeStorage.close();
    return 1;
  }
