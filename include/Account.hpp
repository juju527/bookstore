...
public:
  ...
  int Privilege;
  Account() { Privilege = 0; }
  Account(String30 id, String30 psd, String30 name, int pri) {
    UserID = id, Password = psd, Username = name, Privilege = pri;
  }
...
public:
  ...
  bool login(String30 UserID, String30 Password) {
    ...
    if (Password.getlen()) {
      ...
      if (getAccount().Privilege <= res[0].Privilege)
        return 0;
    }
  ...
  bool changePassword(String30 UserID, String30 CurrentPassword,
                      String30 NewPassword) {
    auto res = BL.find(UserID);
    if (!res.size())
      return 0;
    if (!CurrentPassword.getlen()) {
      if (getAccount().Privilege < 7)
        return 0;
    } else {
      if (!(CurrentPassword == res[0].Password))
        return 0;
    }
    BL.del(UserID, res[0]), res[0].Password = NewPassword,
                            BL.ins(UserID, res[0]);
    return 1;
  }
  bool useradd(String30 UserID, String30 Password, int Privilege,
               String30 Username) {
    if (getAccount().Privilege <= Privilege)
      return 0;
    auto res = BL.find(UserID);
    if (res.size())
      return 0;
  ...
  bool deleteUser(String30 UserID) {
    ...
    for (int i = 0; i < len; i++)
      if (UserStack[i].first.UserID == UserID)
        return 0;
    BL.del(UserID, res[0]);
    return 1;
  }
