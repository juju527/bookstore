...
public:
  ...
  Book(int id, String20 isbn, String60 bookname, String60 author,
       String60 keyword, int num, double pri) {
    Bookid = id, ISBN = isbn, BookName = bookname, Author = author,
    Keyword = keyword, number = num, price = pri;
  }
  ...
  friend std::ostream &operator<<(std::ostream &out, const Book &book) {
    ...
    out << book.Keyword;
    out << '\t';
    out << fixed << setprecision(2) << book.price;
    out << '\t';
    out << book.number;
    return out;
  }
...
public:
  ...
  int select(String20 ISBN) {
    ...
    if (!res.size()) {
      books.open();
      int num = 0;
      books.get_info(num, 1);
      num++;
      books.write_info(num, 1);
      Book cur;
      cur.Bookid = num;
      cur.ISBN = ISBN;
      books.writeorder(cur, num);
      books.close();
      BL1.ins(ISBN, num);
      BL2.ins(String60(), num);
  ...
  bool modifyISBN(String20 ISBN) {
    ...
    if (res.size())
      return 0;
    BL1.del(cur.ISBN, current);
    cur.ISBN = ISBN;
    BL1.ins(cur.ISBN, current);
  ...
  bool modifyBookName(String60 BookName) {
    ...
    Book cur;
    books.open(), books.readorder(cur, current), books.close();
    BL2.del(cur.BookName, current), cur.BookName = BookName,
                                    BL2.ins(cur.BookName, current);
    books.open(), books.writeorder(cur, current), books.close();
    return 1;
  }
  bool modifyAuthor(String60 Author) {
    ...
    Book cur;
    books.open(), books.readorder(cur, current), books.close();
    BL3.del(cur.Author, current), cur.Author = Author,
                                  BL3.ins(cur.Author, current);
    books.open(), books.writeorder(cur, current), books.close();
    return 1;
  }
  bool modifyKeyword(String60 Keyword) {
    ...
    BL4.ins(aux, current), aux = "";
    books.open();
    books.writeorder(cur, current), books.close();
    return 1;
  }
